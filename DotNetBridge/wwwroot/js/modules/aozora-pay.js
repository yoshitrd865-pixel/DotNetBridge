// wwwroot/js/modules/aozora-pay.js

export function initAozoraPay() {
    // 既にQR・口座表示エリアが存在すれば実行しない
    ...
    statusDiv.innerText = '🏦 HHC_Pay: あおぞら銀行口座を発行中...';
    ...
    statusDiv.innerText = `🏦 HHC_Pay: 口座発行APIへ接続中...`;

    const qrContainer = document.createElement('div');
    qrContainer.id = 'tfk-paygate-qr-area';
    qrContainer.style.cssText = 'margin-top: 30px; padding: 20px; border: 2px dashed #2980b9; text-align: center; background: #f8f9fa; border-radius: 8px; width: 95%; margin-left: auto; margin-right: auto; page-break-inside: avoid;';
    qrContainer.innerHTML = '<span style="color:#2980b9; font-weight:bold;">🏦 専用口座（あおぞら銀行）を発行中...⏳</span>';
    ...
    // 接続先をあおぞら用エンドポイントに変更
    const apiUrl = window.location.origin + '/api/AozoraPayment/create-account';
    ...
    .then(data => {
        // バックエンドから口座情報が返ってきた場合
        if(data.accountNumber || data.account_number) {
            statusDiv.innerText = '✅ HHC_Pay: 口座発行成功！';
            statusDiv.style.background = '#27ae60';
            setTimeout(() => statusDiv.remove(), 4000);

            const bankName = data.bankName || "GMOあおぞらネット銀行";
            const branchName = data.branchName || data.branch_name || "";
            const accountNumber = data.accountNumber || data.account_number;
            const accountHolder = data.accountHolder || data.account_holder || "ハシモトハイツ";

            // 振込情報をテキスト化してQRコード化
            const qrText = `振込先:${bankName} ${branchName} ${accountNumber} ${accountHolder}`;
            const qrImageUrl = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(qrText)}`;
            ...
            qrContainer.innerHTML = `
                <div style="display:flex; align-items:center; justify-content:center; gap:25px; padding:10px;">
                    <div><img src="${qrImageUrl}" style="width:130px; height:130px;"></div>
                    <div style="text-align: left;">
                        <h3 style="margin:0 0 6px 0; color:#2980b9; font-size:16px;">🏦 銀行振込（専用口座）でお支払い</h3>
                        <p style="margin:0; font-size:13px; color:#333; line-height:1.6;">
                            <strong>振込先:</strong> ${bankName} ${branchName} (普通) <strong>${accountNumber}</strong><br>
                            <strong>口座名義:</strong> ${accountHolder}<br>
                            <strong style="color:#c0392b; font-size:16px; display:inline-block; margin-top:4px;">ご請求金額: ${amount.toLocaleString()} 円</strong>
                        </p>
                    </div>
                </div>
            `;
        } else {
            statusDiv.innerText = '⚠️ エラー: 口座発行失敗';
            statusDiv.style.background = '#c0392b';
            qrContainer.innerHTML = `<span style="color:#c0392b; font-weight:bold;">⚠️ 口座発行エラー: ${data.error || 'レスポンス異常'}</span>`;
        }
    })
    .catch(err => {
        statusDiv.innerText = '⚠️ エラー: サーバー通信失敗';
        statusDiv.style.background = '#c0392b';
        qrContainer.innerHTML = `<span style="color:#c0392b; font-weight:bold;">⚠️ サーバー通信失敗 (バックエンドの確認が必要です)</span>`;
    });
}